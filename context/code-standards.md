...
- Components run on the client by default. Use `createServerFn` for server-side logic.
- API routes live under `app/routes/api/` and should remain focused on a single responsibility.
- Long-running work belongs in background tasks, not in API route handlers.
...
- Canvas snapshots and generated specs belong in Cloudflare R2; Prisma stores only the R2 object key reference.
