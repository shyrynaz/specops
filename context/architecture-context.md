...
| Artifact storage | Cloudflare R2           | Canvas snapshots and generated Markdown specs                  |
...
- **Cloudflare R2**: generated artifacts — canvas snapshots at `canvas/{projectId}.json` and specs at `specs/{projectId}/{specId}.md`.
- Project records, spec records, and task run records belong in PostgreSQL.
- Canvas content and Markdown output are stored in and retrieved from Cloudflare R2.
- The R2 object key is stored in the database (`canvasJsonPath`, `filePath`) as the reference to the artifact.
